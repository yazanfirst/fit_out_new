...
export async function getTimelineByProjectId(projectId: string): Promise<TimelineMilestone[]> {
  try {
    ...
    return (data || []) as unknown as TimelineMilestone[];
  } catch (error) {
...
export async function createMilestone(milestone: Omit<TimelineMilestone, 'id' | 'created_at' | 'updated_at'>): Promise<TimelineMilestone | null> {
  try {
    // Ensure planned_start is set
    if (!milestone.planned_start) {
      milestone.planned_start = milestone.planned_date;
    }
...
export async function updateMilestone(id: string, milestone: Partial<TimelineMilestone>): Promise<TimelineMilestone | null> {
  try {
    // Ensure planned_start is set if planned_date is updated
    if (milestone.planned_date && !milestone.planned_start) {
      milestone.planned_start = milestone.planned_date;
    }
