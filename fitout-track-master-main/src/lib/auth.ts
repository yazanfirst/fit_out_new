    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (username.toLowerCase() === 'admin' && password === 'admin123') {
        ...
        try {
          ...
          await createAuditLog(user.id, 'login', 'session', user.id, { details: 'User logged in (dev mode)' });
        } catch (auditError) {
        ...
        if (user) {
          ...
          if (storedPassword) {
            if (password === storedPassword) {
              ...
              try {
                await createAuditLog(user.id, 'login', 'session', user.id, { details: 'Dev user logged in' });
              } catch (auditError) {
    ...
    if (process.env.NODE_ENV === 'production') {
      try {
        ...
        const { data: sessionData, error: sessionError } = await supabase.auth.signInWithPassword({
          email: `${username}@fitouttrack.example`, // Using username as email since we don't collect emails
          password
        });
        
        if (sessionError) {
          console.warn('Supabase Auth signIn error:', sessionError);
          // Continue anyway, as we've already verified the password with bcrypt
        }
      } catch (authError) {
        console.warn('Supabase Auth error (non-critical):', authError);
        // Continue anyway, as we've already verified the password with bcrypt
      }
    }

    // Return user without password hash and with normalized role casing
    const user: User = {
      id: userData.id,
      username: userData.username,
      role: (userData.role as string).charAt(0).toUpperCase() + (userData.role as string).slice(1).toLowerCase() as UserRole,
      created_at: userData.created_at,
      updated_at: userData.updated_at
    };

    // Store authenticated user in localStorage for any environment
    storage.setItem('authenticated_user', JSON.stringify(user));

    // Log successful login
    try {
      await createAuditLog(user.id, 'login', 'session', user.id, { details: 'User logged in' });
    } catch (auditError) {
...
export async function logoutUser(): Promise<{ error: string | null }> {
  try {
    // Always remove from localStorage in any environment
    storage.removeItem('authenticated_user');
    
    // In production, also sign out from Supabase Auth
    if (process.env.NODE_ENV === 'production') {
      try {
        ...
        console.warn('Supabase Auth signOut error (non-critical):', authError);
        // Continue anyway as we've already cleared localStorage
      }
    }
...
export async function getCurrentUser(): Promise<User | null> {
  try {
    // For both development and production, try localStorage first for better performance
    const storedUser = storage.getItem('authenticated_user');
    if (storedUser) {
      const parsedUser = JSON.parse(storedUser);
      // Normalize role casing to ensure consistency
      if (parsedUser.role) {
    ...
    // In production, if no localStorage user, check Supabase auth
    if (process.env.NODE_ENV === 'production') {
      console.log('Production mode: checking Supabase auth session');
      ...
      // Create a normalized user object with proper role casing
      const user: User = {
        id: userData.id,
        username: userData.username,
        role: (userData.role as string).charAt(0).toUpperCase() + (userData.role as string).slice(1).toLowerCase() as UserRole,
      ...
      // Store in localStorage for faster subsequent access
      storage.setItem('authenticated_user', JSON.stringify(user));
