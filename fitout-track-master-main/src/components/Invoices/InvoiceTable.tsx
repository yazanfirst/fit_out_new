...
const InvoiceTable: React.FC<InvoiceTableProps> = ({ projectId }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const invoiceData: Omit<Invoice, 'id' | 'created_at' | 'updated_at'> = {
      ...
      file_path: editingInvoice?.file_path || ''
    };
                ...
                {filteredInvoices.length === 0 ? (
                  ...
                  filteredInvoices.map((invoice) => (
                    <TableRow key={invoice.id}>
                      ...
                      <TableCell>
                        {(invoice.fileUrl || invoice.file_path) ? (
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 w-7 p-0"
                            onClick={async () => {
                              let url = invoice.fileUrl;
                              if (!url && invoice.file_path) {
                                // Dynamically get the file URL if missing
                                const bucketName = 'invoice-files';
                                const filePath = invoice.file_path.replace(`${bucketName}/`, '');
                                // getStorageUrl is imported already
                                url = await getStorageUrl(bucketName, filePath);
                              }
                              if (url) {
