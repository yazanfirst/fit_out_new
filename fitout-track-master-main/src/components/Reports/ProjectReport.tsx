...
// Helper to safely create Date objects (replicated from TimelineView)
...
// Delay calculation logic (replicated and adapted from TimelineView)
const isMilestoneDelayed = (milestone: TimelineMilestone): boolean => {
  ...
  if (milestone.status === 'Not Started' && plannedStart && today > plannedStart) return true;
  if (milestone.status === 'In Progress' && plannedCompletion && today > plannedCompletion) return true;
...
const ProjectReport: React.FC<ProjectReportProps> = ({ 
  ...
  // Use the refined delay calculation
  const delayedMilestonesCount = milestones.filter(isMilestoneDelayed).length;
    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardTitle className={`text-2xl ${delayedMilestonesCount > 0 ? 'text-red-600' : 'text-green-600'}`}>
            {delayedMilestonesCount > 0 ? `${delayedMilestonesCount} Delayed` : 'On Schedule'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          {delayedMilestonesCount > 0 && (
            <p className="text-xs text-red-500 mt-1">({delayedMilestonesCount} currently delayed)</p>
          )}
        </CardContent>
         <CardFooter>
