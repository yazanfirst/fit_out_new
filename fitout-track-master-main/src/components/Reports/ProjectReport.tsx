...
import { getDrawingsByProjectId } from '@/lib/api';
...
const ProjectReport: React.FC<ProjectReportProps> = ({ 
  ...
  const handleExportPDF = async () => {
    try {
      ...
      // Fetch project photos if available
      let projectPhotos: Drawing[] = [];
      try {
        projectPhotos = await getDrawingsByProjectId(project.id);
        // Filter to keep only photos (not drawings)
        projectPhotos = projectPhotos.filter(item => item.type === 'Photo');
      } catch (error) {
        console.error('Error fetching project photos:', error);
        // Continue without photos if there was an error
      }
