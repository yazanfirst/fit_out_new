...
const ProjectDetails = () => {
  ...
  const { hasPermission, user, userProjects, isContractor } = useAuth();
  ...
  // Check if contractor has access to this project
  useEffect(() => {
    if (isContractor && id && userProjects.length > 0) {
      if (!userProjects.includes(id)) {
        toast.error('You do not have access to this project');
        navigate(`/project/${userProjects[0]}`);
      }
    }
  }, [isContractor, id, userProjects, navigate]);
