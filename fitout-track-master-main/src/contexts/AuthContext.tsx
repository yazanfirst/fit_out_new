      ...
      try {
        ...
        if (currentUser) {
          if (currentUser.role.toLowerCase() === 'contractor') {
            ...
            // Only redirect if contractor is on the dashboard and has projects
  ...
  const login = async (username: string, password: string): Promise<boolean> => {
    try {
      ...
      if (loggedInUser.role.toLowerCase() === 'contractor') {
        ...
        // Check if contractor has any projects assigned
        if (projects.length === 0) {
          toast({
            title: 'No projects assigned',
            description: 'You have no projects assigned to your account. Please contact your administrator.',
            variant: 'destructive'
          });
          return false;
        }
        
        // If contractor has only one project, redirect to it
        if (projects.length === 1) {
          navigate(`/project/${projects[0]}`);
        } else {
          // If contractor has multiple projects, redirect to the first one but they can switch using the project selector
          navigate(`/project/${projects[0]}`);
          toast({
            title: 'Multiple projects assigned',
            description: `You have ${projects.length} projects assigned. Use the project selector in the navigation to switch between projects.`,
            variant: 'default'
          });
        }
      } else {
